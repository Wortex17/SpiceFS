exports.generateUUID = function generateUUID() {
    ...
    if(!generateUUID.runningID)
        generateUUID.runningID = 1;
    else
        generateUUID.runningID++;

    var iteratingPart = (Math.floor(Math.random() * 100) * 1000 + generateUUID.runningID).toString(16);
    ...
    return head.concat(iteratingPart, generateUUID.tail).join('-');
};